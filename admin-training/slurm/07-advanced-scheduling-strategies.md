...
-->
In this hands-on, you will be able to setup advanced scheduling strategies which will allow you to increase even more the cluster utilisation and also achieve even more fairness access to the shared resources.
...
Job preemption allows to stop one or more "low-priority" jobs to let a "high-priority" job run. Different job preemption mechanisms can be implemented: job suspension, job checkpointing and restart and job cancel.
With job preemption, the *cluster domination by long term jobs can be mitigated*. By adopting this strategy, you should be able to schedule a really large (and high-priority) jobs quite quickly by suspending other low priority jobs.
...
A combination of large number of short single core jobs (including parametric jobs) waiting in the partition, the efficiency of the cluster could grow easily to 90% of utilisation.
...
Several HPC facilities see the Checkpointing & Restart (C&R) as a "resilience" mechanism, since you can restart the job from the last checkpoint and save several CPUTime in case of disaster. HPCNow! strongly suggest to consider to ban running long jobs (than ~7 days) if the code doesn't have checkpointing mechanism or if it's not capable to use 3rd-party C&R software ([SCR](https://computation.llnl.gov/projects/scalable-checkpoint-restart-for-mpi), [DMTCP](http://dmtcp.sourceforge.net/), etc.).
...
In addition to the mitigation of cluster monopolisation, this could potentially minimise job scattering and achieving this way reduce the latency overhead on the MPI jobs. Having said that, C&R is a very expensive operation because it usually involves moving large datasets in the cluster file system. For that reason, the scheduler should be configured in order to reduce excessive use of this mechanism.
...
The following partition schema meets the 90% of regular requirements. If you don't have clarity about your user requirements, the following suggested configuration will give you a good starting point:
...
* PriorityTier, allows to define a suspension priority. Unfortunately, partition's priority tier takes precedence over a job's priority.
...
In order to avoid to expose the requeue partition by default to all the users, you can export the following variable in the user environment. Otherwise, you can also consider to develop a submit plugin to route the jobs requesting "requeue" (```SBATCH --requeue```).
...
Submit a short term job (high priority partition) requiring one of the nodes allocated for previous job. i.e.: --nodelist=skl[001-010] in order to force to suspend the low priority job.
```
...
Once the job is running, submit another one requiring one of the nodes allocated for previous job in order to force preemption.
```
...
The fairsharing mechanism will regulate the priority of the jobs based on who has used more or less resources after each cycle. While short term jobs are nicely handled by backfilling and fair sharing priority; small and long term jobs can dominate the cluster and lock down the resources for other users for very long period of time. In order to mitigate this risk, you can implement job preemption as detailed above. In addition to that, HPCNow! suggest to consider to setup upper limits per user and/or group accounts in order to restrict the resources available at any given time in the long term jobs. In this scenario (partition based preemption), QoS must be associated with the partitions with Partition QOS (or AllowQos + custom plugin which is not covered in this hands-on).

Partition QoS allows to define the limits that a traditional definition of a partition is not able to do. All the nodes can be assigned to a partition and then in the Partition's QOS limit the number of resources available for that partition (i.e. GrpCPUs or GrpNodes).
